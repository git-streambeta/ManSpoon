...
function updatePlayer() {
  ...
  if (teleportActive && isOutsideMap(nx, ny)) {
    tryTeleportPlayer(nx, ny, now);
    return;
  }
  ...
  // ===== СБОР БОНУСОВ (возвращаем твой код) =====
...
function smoothMovePlayer() {
  if (!VISUAL_CONFIG.smoothMovement) return;
...
function tryTeleportPlayer(nx, ny, now) {
  const targetX = nx < 0 ? W - 1 : nx >= W ? 0 : nx;
  const targetY = ny < 0 ? H - 1 : ny >= H ? 0 : ny;
  ...
  spawnTeleportEffect(g.p.x, g.p.y, '#00f5ff');
  ...
  setTimeout(() => {
    ...
    spawnTeleportEffect(targetX, targetY, '#00ffaa');
...
function spawnTeleportEffect(x, y, color) {
  for (let i = 0; i < 20; i++) {
    g.parts.push({
      x: x * CELL + CELL / 2,
      y: y * CELL + CELL / 2,
...
function spawnTrailParticle(x, y) {
  if (!PLAYER_CONFIG.trailParticles) return;
...
function checkDoorEntry(nx, ny) {
  if (!g.doorVis || !g.doorPos) return;
  if (nx !== g.doorPos.x || ny !== g.doorPos.y) return;

  const isEarlyLevel = LEVEL_CONFIG.exitConditions.earlyExit.includes(g.level);
  const canExit = isEarlyLevel || g.bots.length === 0;

  if (!canExit || g.state !== 'playing') return;
  ...
  // ❗ ВОТ ТА САМАЯ ЗАДЕРЖКА, КОТОРАЯ ПРОПАЛА
  setTimeout(() => {
    g.state = 'win';
    g.score += g.level * LEVEL_CONFIG.levelClearBonus;
...
function devWinLevel() {
  ...
  showModal('DEV MODE: УРОВЕНЬ ПРОЙДЕН', g.score, 'ДАЛЬШЕ', () => {
    g.level++;
    init();
  });
}
...
function showModal(title, score, btnText, callback) {
  // ✅ Вычитаем штраф
  const timePenalty = SCORING_CONFIG.timePenalty ? gameTime : 0;
  ...
  // ✅ ПОКАЗЫВАЕМ штраф ТОЛЬКО при поражении
  if (SCORING_CONFIG.timePenalty && title === 'ПОРАЖЕНИЕ') {
    html += `<span style="color:#ff6666;">ШТРАФ ЗА ВРЕМЯ: -${timePenalty}</span><br>`;
    html += `<span class="final-score" style="color:#00ff88;font-size:18px;">ИТОГО: ${finalScore} ОЧКОВ</span><br>`;
  }
  
  if (title !== 'ПАУЗА') {
    html += `<br><div style="font-size:16px;color:#8899aa;margin-top:15px;">ТОП-3 РЕКОРДА:</div>`;
    bestScores.forEach((s, i) => {
      html += `<div style="font-size:14px;margin:5px 0;">${i + 1}. ${s.score} очков (ур. ${s.level})</div>`;
    });
  }
  ...
  document.getElementById('modalBtn').onclick = () => {
    ...
    // ✅ ПРИМЕНЯЕМ штраф к реальным очкам (всегда, не только при победе)
    if (title !== 'ПАУЗА' && SCORING_CONFIG.timePenalty) {
      g.score = finalScore;
      updateUI(); // ✅ ОБНОВЛЯЕМ панель!
    }
